...
type Inhib struct {
	Layer     FFFBPars `desc:"inhibition across the entire layer"`
	UnitGroup FFFBPars `desc:"inhibition across groups of units, for layers with 4D shape"`
}
...
// relevant scope
func (fb *FFFBPars) FFInhib(avgGe, maxGe float32) float32 {
...
func (fb *FFFBPars) FBInhib(avgAct float32) float32 {
	fbi := fb.FB * avgAct
	return fbi
}

// FBUpdt updates feedback inhibition using time-integration rate constant
func (fb *FFFBPars) FBUpdt(fbi *float32, newFbi float32) {
	*fbi += fb.FBDt * (newFbi - *fbi)
}

func (fb *FFFBPars) Update() {
	fb.FBDt = 1 / fb.FBTau
}

func (fb *FFFBPars) Defaults() {
	fb.Gi = 1.8
	fb.FF = 1
	fb.FB = 1
	fb.FBTau = 1.4
...
// LayActAvg represents expected average activity levels in the layer.
...
type LayActAvg struct {
	...
	Tau       float32 `condshow:"Fixed=false" def:"100" min:"1" desc:"time constant in trials for integrating time-average values at the layer level -- used for computing acts_m_avg and acts_p_avg"`
...
func (la *LayActAvg) Update() {
	la.Dt = 1 / la.Tau
}

func (la *LayActAvg) Defaults() {
	la.Init = 0.15
	la.Fixed = false
	la.UseExtAct = false
	la.UseFirst = true
