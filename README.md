...
* There is also a `leabra.LeabraLayer` and `leabra.LeabraPrjn` interface, defined in leabra/leabra/leabra.go, which provides a virtual interface for the Leabra-specific algorithm functions at the basic level.  These interfaces are used in the base leabra code, so that any more specialized version that embeds the basic leabra types will be called instead.  See `leabra/deep` for implemented example that does DeepLeabra on top of the basic `leabra/leabra` foundation.

* Layers have a `Shape` property, using the `etensor.Shape` type (see below), which specifies their n-dimensional (tensor) shape.  Standard layers are expected to use a 2D Y*X shape (note: dimension order is now outer-to-inner or *RowMajor* now), and a 4D shape then enables `Pools` ("unit groups") as hypercolumn-like structures within a layer that can have their own local level of inihbition, and are also used extensively for organizing patterns of connectivity.
...
* `python` contains a `Makefile` that uses [GoPy](https://github.com/goki/gopy) to generate python bindings to the entire emergent system.  `examples/leabra25ra/ra25.py` is a python version of the `ra25.go` code that runs the example entirely in python!  See the `README.md` file in that directory for more specific information.
