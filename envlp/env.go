...
// the temporal state of the environment, using etime.Times standard
// intervals.
//
...
type Env interface {
	...
	// Mode returns the time.EvalMode for this environment (Train, Test, etc)
	// This is used for setting the Scope for the counters.
	Mode() string
	...
	// Counter returns counter for given scope.  If not found, returns nil.
	Counter(scope time.ScopeKey) *Ctr
