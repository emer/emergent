...
// which we modified only to use the erand.Rand interface.
...
func BinomialGen(n, p float64, thr int, randOpt ...Rand) float64 {
	...
	for {
		var em, y float64
		for {
			y = math.Tan(math.Pi * rnd.Float64(thr))
			em = sq*y + am
			if em >= 0 && em < n+1 {
				break
			}
...
// a shape parameter (Alpha, Par in RndParams),
// and a scaling parameter (Beta, Var in RndParams).
// Thr is an optional parallel thread index (-1 for none).
// Optionally can pass a single Rand interface to use --
// otherwise uses system global Rand source.
func GammaGen(alpha, beta float64, thr int, randOpt ...Rand) float64 {
	...
	case a < smallAlphaThresh:
		...
		for {
			e := rnd.ExpFloat64(thr)
			var z float64
			if e >= -lr {
				z = e + lr
			} else {
				z = -rnd.ExpFloat64(thr) / lambda
			}
			eza := math.Exp(-z / a)
			lh := -z - eza
			var lEta float64
	...
	case a >= smallAlphaThresh:
		...
		if a < 1 {
			d += 1.0
			m = math.Pow(rnd.Float64(thr), 1/a)
		}
		c := 1 / (3 * math.Sqrt(d))
		for {
...
func GaussianGen(mean, sigma float64, thr int, randOpt ...Rand) float64 {
	...
	return mean + sigma*rnd.NormFloat64(thr)
}
...
func BetaGen(alpha, beta float64, thr int, randOpt ...Rand) float64 {
	...
	ga := GammaGen(alpha, 1, thr, rnd)
	gb := GammaGen(beta, 1, thr, rnd)
